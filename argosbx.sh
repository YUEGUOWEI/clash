...
up(){
rm -rf /usr/bin/agsb
curl -L -o /usr/bin/agsb -# --retry 2 --insecure https://raw.githubusercontent.com/YUEGUOWEI/ArgoSB/refs/heads/main/argosbx.sh
...
sed -i '/YUEGUOWEI/d' ~/.bashrc
echo "export nix=y uuid=${uuid} vmpt=${port_vm_ws} agn=${ARGO_DOMAIN} agk=${ARGO_AUTH} && bash <(curl -Ls https://raw.githubusercontent.com/YUEGUOWEI/ArgoSB/refs/heads/main/argosbx.sh)" >> ~/.bashrc
source ~/.bashrc
sleep 2
